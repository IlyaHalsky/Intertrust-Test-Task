...
import com.intertrust.protocol.{Event, PersistableEvent, PersistableState, TimeTick}

import java.time.Instant
import scala.annotation.tailrec

object PseudoKafkaConsumer {
  def apply[Message <: Event](
    actorName: String,
    eventQueue: Iterator[Message],
    sendTo: ActorRef[Message]*
  ): Behavior[TimeTick] =
    StatefulPersistentBehaviour(PseudoKafkaConsumer(actorName, _, eventQueue, sendTo.toList))
}

case class ConsumedMessage[Message <: Event](offset: Int, nextMessage: Option[Message]) extends PersistableEvent

case class CurrentOffset[Message <: Event](offset: Int, nextMessage: Option[Message]) extends PersistableState

case class PseudoKafkaConsumer[Message <: Event](actorName: String, context: ActorContext[TimeTick], eventQueue: Iterator[Message], sendTo: List[ActorRef[Message]])
  ...
  def commandHandler(state: CurrentOffset[Message], command: TimeTick): Effect[ConsumedMessage[Message], CurrentOffset[Message]] =
    if (state.nextMessage.exists(_.timestamp.isAfter(command.time))) Effect.none
      ...
      Effect.persist(event).thenRun(_ => messages.foreach(msg => sendTo.foreach(_ ! msg)))
    }
